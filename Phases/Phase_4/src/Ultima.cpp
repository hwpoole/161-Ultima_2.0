...
void Tick() {
  ...
  if (UFSPtr != nullptr) {
    write_window_fast(UFS_Win, UFSPtr->Dump().c_str());
  }
...
void tasks_write_without_creating_a_file(void *args) {
  TaskContext *Context = (TaskContext *)args;
  WINDOW *Window = Context->win;
  const char *Name = Context->name;
  char buffer[256];

  write_window(Window, " Write to a file...\n");
  SchedulerPtr->yield();
  ...
  sprintf(buffer, " %s Writes to %s\n", Name, Random_Name);
  write_window(Window, buffer);
  for (int i = 0; i < 10; i++) {
    char random_char = 'a' + rand() % 26;

    if (UFSPtr->Write_Char(Random_Name, random_char) != -1) {
      write_window(Window, " Success!\n");
    } else {
      write_window(Window, " Failed\n");
      break;
    }
  }

  sprintf(buffer, " %s needs to create \n %s\n", Name, Random_Name);
  write_window(Window, buffer);
}
...
void tasks_create_a_file_and_write_to_it(void *args) {
  ...
  string file;
  int Permissions[4] = {0, 0, 0, 0};

  write_window(Window, " Create a file...\n");
  SchedulerPtr->yield();

  int ID = SchedulerPtr->get_task_id();
  switch (ID) {
  case '1': {
    file = "File_1";
    int NewPermissions[4] = {0, 6, 6, 6};
    memcpy(Permissions, NewPermissions, sizeof(NewPermissions));
    break;
  }
  case '2': {
    file = "File_2";
  ...
  strncpy(FileName, file.c_str(), 7);

  sprintf(buffer, " %s creates %s\n", Name, FileName);
  if (UFSPtr->Create_File(FileName, 10, Permissions) == -1) {
    sprintf(buffer, " Failed to create %s\n", file.c_str());
    write_window(Window, buffer);
  ...
  sprintf(buffer, " %s opens %s\n", Name, FileName);
  if (UFSPtr->Open(FileName, ufs::W) == -1) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (UFSPtr->Write_Char(FileName, rand_char) == -1) {
      sprintf(buffer, " Failed writing.\n");
      write_window(Window, buffer);
      return;
    } else {
      sprintf(buffer, " Wrote %c\n", rand_char);
      write_window(Window, buffer);
    }
  }
}
 ...
 * should be able to write to it. Task 3 will try to write beyond the
 * possible bounds of its file.
 */
void tasks_try_to_write_files_they_dont_own(void *args) {
  TaskContext *Context = (TaskContext *)args;
  WINDOW *Window = Context->win;
 ...
 * A helper method for scenario 5.
 *
 * In this sub-scenario, each task will try to make a file that is way too
 * big for our current free space. We expect all to error out.
 *
